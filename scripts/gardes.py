...
def garde_non_reecriture(fait, texte_source_brut, seuil_recouvrement=0.3, source_attendue=None):
    """
    ...
    Si source_attendue est fourni et que la source du FAIT est différente,
    la GARDE retourne non_verifiable (le texte brut ne correspond pas à
    cette source — vérification impossible, pas violation).

    fait: dict avec "valeur" et "origine": {"source":...}
    texte_source_brut: str, contenu brut de sources_raw pour ce champ
    source_attendue: str, source que represente texte_source_brut (ex: "metiers_quebec")
    """
    valeur = str(fait.get("valeur", "")).lower()

    # Si la source du FAIT ne correspond pas à celle du corpus brut,
    # la vérification n'est pas applicable — non_verifiable, pas violation.
    if source_attendue:
        source_fait = fait.get("origine", {}).get("source", "")
        if source_fait != source_attendue:
            return {"conforme": None, "violations": [f"source du FAIT ({source_fait}) != source attendue ({source_attendue}) — non applicable"]}
...
def garde_irreversibilite_corpus(faits):
    """
    Variante statique de garde_irreversibilite : vérifie la cohérence
    interne d'une liste de FAITS déjà stockés (une fiche), sans nécessiter
    un second passage d'ingestion. Détecte le même type de violation
    (écrasement) mais en comparant chaque FAIT à tous les autres de la
    même fiche, a posteriori.

    Complète garde_irreversibilite (qui vérifie un ajout au moment de
    l'ingestion) -- celle-ci vérifie un corpus déjà écrit, ce qui permet
    de l'exécuter dans un smoke test sans ré-exécuter l'ingesteur.

    faits: liste de FAITS d'une même fiche
    """
    violations = []
    vus = {}  # (champ, source, date_captee) -> valeur déjà vue
    for f in faits:
        origine = f.get("origine", {})
        cle = (f.get("champ"), origine.get("source"), origine.get("date_captee"))
        if None in cle:
            continue  # cle incomplete, signalé par garde_autojugement séparément
        if cle in vus and vus[cle] != f.get("valeur"):
            violations.append(
                f"écrasement détecté: champ={cle[0]} source={cle[1]} date={cle[2]} "
                f"valeurs divergentes pour une même clé (champ, source, date_captee)"
            )
        vus[cle] = f.get("valeur")
    return {"conforme": len(violations) == 0, "violations": violations}
